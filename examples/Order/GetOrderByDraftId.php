...
/**
 * Retrieve an Order by its Draft Order's USWerx ID using the USWerx API PHP SDK.
 *
 * Prerequisites:
...
// Determine how to get the Draft Order ID (CLI or HTTP GET)
if (PHP_SAPI === 'cli') {
    // For CLI usage
    if (empty($argv[1])) {
        echo "Please provide a USWerx Draft Order ID as the first argument.\n";
        exit(1);
    }
    $draftOrderId = $argv[1];
} else {
    // For HTTP usage
    if (empty($_GET['draftOrderId'])) {
        echo "Please provide a USWerx Draft Order ID as a 'draftOrderId' query parameter.\n";
        exit(1);
    }
...
try {
    ...
    // Retrieve the Order by draft ID
    $order = Order::findByDraftId($draftOrderId);
